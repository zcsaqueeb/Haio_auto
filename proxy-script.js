...
async function requestChallenge(publicKey) {
    try {
        ...
        const response = await axios.post(
            "https://prod-api.haio.fun/api/auth/request-challenge",
            { publicKey },
...
async function verifyLogin(publicKey, secretKey, challengeMessage) {
    ...
    try {
        await randomDelay();
        const response = await axios.post(
            "https://prod-api.haio.fun/api/auth/verify",
            { publicKey, signature },
            { httpAgent: agent, httpsAgent: agent }
        );
...
async function useReferral(token, account, referralCode) {
    try {
        ...
        const response = await axios.post(
            ...
            { referralCode },
            { headers: { Authorization: `Bearer ${token}` }, httpAgent: agent, httpsAgent: agent }
        );
...
(async () => {
    console.log(chalk.green("🔹 Running Proxy Script with Proxy Retrieved from proxy.txt..."));
    console.log(chalk.green("🔹 Using 100 wallets for referral process."));
    ...
    for (let i = 0; i < 100; i++) {
        ...
        console.log(chalk.green(`🔄 Applying referral ${i + 1}/100 with code: ${referralCode}...`));
        const account = { publicKey, secretKey };
        await useReferral(token, account, referralCode);
